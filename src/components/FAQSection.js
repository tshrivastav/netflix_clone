...
const faqData = [
  {
    ...
    body: (
      <p className="md:text-3xl">
        ...
        single ad – all for one low monthly price. There's always something new
...
const FAQSection = () => {
  return (
    <div className="bg-black px-5 py-10 md:px-40 items-center">
      <div className="container max-w-3xl mx-auto">
        <p className="text-white text-center mb-10 text-2xl md:text-6xl ">
          Frequently Asked Questions
        </p>
        {faqData.map(({ title, body }, i) => (
          <FAQ key={i} title={title} body={body} />
        ))}
        <Search />
      </div>
    </div>
